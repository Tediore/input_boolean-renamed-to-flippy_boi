...
class FlippyBoiStorageCollection(collection.StorageCollection):
    """Flippy boi collection stored in storage."""
...
async def async_setup(hass: HomeAssistantType, config: ConfigType) -> bool:
    ...
    collection.attach_entity_component_collection(
        component, yaml_collection, lambda conf: FlippyBoi(conf, from_yaml=True)
    )

    storage_collection = FlippyBoiStorageCollection(
        Store(hass, STORAGE_VERSION, STORAGE_KEY),
        logging.getLogger(f"{__name__}_storage_collection"),
        id_manager,
    )
    collection.attach_entity_component_collection(
        component, storage_collection, FlippyBoi
    )
    ...
    async def reload_service_handler(service_call: ServiceCallType) -> None:
        """Remove all flippy bois and load new ones from config."""
...
class FlippyBoi(ToggleEntity, RestoreEntity):
